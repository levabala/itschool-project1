...
def fit(X_raw, Y, coeffs_count=15, population_size_start=30, random_injection_each_round=2, best_select_amount=8, mutation_threshold=0.1, iterations_max=200):
  # plt.subplot(221)
  ...
  def compressImage(img, color_range_original=255):
    ...
    for y in range(initial_size):
      for x in range(initial_size):
        average_value += img[y][x]

    average_value /= initial_size ** 2
    ...
    for y in range(coeffs_count):
      ...
      for x in range(coeffs_count):
        ...
        average_around_fit_color_range = 1 if average_around > average_value * 0.3 else 0
  ...
  def mutate(chro):
    return list(map(lambda v: randomWeight() if rnd.random() > mutation_threshold else v, chro))

  def fitForNumber(X, number: int, population=[randomChromosome() for i in range(population_size_start)], iters_left=iterations_max, error_min_previous=None):
    ...
    print("population_size: {}, error_min: {:2.2f}, delta: {:2.8f}".format(len(population),
    ...
    if iters_left <= 0 or error_min < 10e-3:
